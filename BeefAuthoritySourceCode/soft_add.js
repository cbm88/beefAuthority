...
function getLineItemHTML(product, lastAddedIndex, isAnchored) {
	...
	product.ImageSource == undefined ? imageSource = '' : imageSource = '<img src="' + product.ImageSource + '" alt="' + product.ProductName + '" title="' + product.ProductName + '" />'; //height="50px" width="50px"
    ...
    if (!isAnchored) {
        ...
        productHTML += '"><div class="product-image icon"><a href="/ProductDetails.asp?ProductCode=' + product.ProductCode + '" title="' + productName + '" alt="' + productName + '">' + imageSource + '</a></div>';
...
function hideCart() {
	if (displayCartPopup) {
	    displayCartPopup.Dispose();
	    document.getElementsByTagName("html")[0].classList.remove("SoftAddToCartOn");
	}
    if (cartDiv != null) {
...
DisplayCartPopup.prototype.Show = function() {
    ...
    if (this.ShowOK) {
        ...
        document.getElementsByTagName("html")[0].classList += " SoftAddToCartOn";
        return;
    }
    var instance = this;
